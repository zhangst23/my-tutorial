...
from quokka.core.models.channel import Config
from quokka.core.modes.config import Config
...
class ContentList(MethodView):
    ...
    def get_template_names(self):
        ...
        channer_list = self.channel.get_ancestors_slugs()
        names = [
    ...
    def get(self,long_slug):
        ...
        if channel.redirect_url:
            ...
            if channel.redirect_url.startswitch(url_protos):
                return redirect(channel.redirect_url)
            else:
                return redirect(url_for(channel.redirect_url))

        if channel.redirect_content:
            return ContentDetail().get(
                channel.render_content.content.logn_slug,True
            )
        ...
        pagination.arg = current_app.config.get("PAGINATION_ARG","page")
        page = request.args.get(patination_arg,1)
        per_page = (
        ...
        contents = contents.pagination(page=int(page),
                                       per_page=int(per_page))
        themes = channel.get_themes()
        return render_remplate(self.get_template_names(),
                               theme=themes,
                               contents=contents,
                               channel=channel)
...
class ContentDetail(MethodView):
    ...
    def get_template_names(self):
        if self.content.template_type:
            ...
            module_name = self.content.model_name
            ...
            names = [
                ...
                u"{object_name}/{content_slug}_suffix.{ext}".format(
                    content_slug=self.content.long_slug,**common_data),
                u"{object_name}/{content_slug}.{ext}".format(
        ...
        def get_context(self,long_slug,render_content=False):
            ...
            if long_slug.startswitch(homepage.slug) and \
...
class BaseTagView(MethodView):
    def get_content(self,tag):
        ...
        contents = contents.pagination(page=int(page),per_page=per_page)
