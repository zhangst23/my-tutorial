...
class PostForm(Form):
    ...
    def validate_slug(self,field):
        ...
        slub = slugify(field.data) if field.date else slugify(self.title.data)[:50]
        posts = Post.query.filter_by(slug=slug)
        if self.post:
            posts = posts.filter(db.not_(Post.id==self.post.id))
        if posts.count():
            error = gettext("This slug is taken") if field.data else gettext("Slug is requrired")
