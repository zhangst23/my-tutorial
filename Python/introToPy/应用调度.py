...
组合应用¶
如果你想在同一个 Python 解释器中运行多个独立的应用，那么你可以使用 werkzeug.wsgi.DispatcherMiddleware 。其原理是：每个独立的 Flask 应用都 是一个合法的 WSGI 应用，它们通过调度中间件组合为一个基于前缀调度的大应用。
