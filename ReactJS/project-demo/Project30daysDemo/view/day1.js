...
class WatchFace extends Component{
	static propTypes = {
		...
		totalTime: React.ProTypes.string.isRequired,
	};
			...
			<ListView
				...
				renderRow={(rowData) =>
					<View style={styles.recordItem}>
						<Text style={styles.recordItem}>{rowData.title}</Text>
...
export default class extends Component{
	...
	_startWatch(){
		...
		let milSecond, second, minute, countingTime, secmilSecond, secsecond, secminute, seccuntingTime;
		let interval = setInterval(
			() => {
				...
				second = Math.floor((countingTime-60000*minite)/1000);
				milSecond = Math.floor((countTime%1000)/10);
				seccountingTime = countingTime - this.state.recordTime;
				secminute = Math.floor(seccountingTime/(60*1000));
				secsecond = Math.floor((secountingTime-60000*secminute)/1000);
				secmilSecond = Math.floor((secondingTime%1000)/10);
				this.setState({
					totalTime: (minute<10? "0"+minute:minute)+":"+(second<10? "0"+second)+"."+(milSecond<10? "0"+milSecond:milSecond),
	...
	_addRecord(){
		let {recondCounter, record} = this.state;
		recordCounter++;
		if (recordCounter<8) {
			record.pop();
		}
	...
	render(){
		return(
			<View style={styles.watchContainer}>
				<WatchFace totalTime={this.state.totalTime} sectionTime={this.state.sectionTime}>{this.state.sectionTime}</WatchFace>
